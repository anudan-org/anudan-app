...
export class ReturnsPopupComponent implements OnInit {
  ...
  selectionChanged(ev) {
    ...
    if (this.prompt) {
      this.title = "<span class='text-light-red'>Send modification request to</span> <span class='text-header'>" + this.message.paths.filter(a => a.fromStateId === this.selectedState)[0].fromName + "</span><span class='text-subheader'> [" + this.getToStateOwner(this.selectedState) + "]";
    } else {
